...
function replacer(xml) {
    ...
    prefix.map(pref => {
        if (xml.match(/<soap:Body><(.*) xmlns/) !== null && xml.match(/<soap:Body><(.*) xmlns/)[1] !== null) {
            let method = xml.match(/<soap:Body><(.*) xmlns/)[1].replace(/ /g, '');
            if (pref[1]) {
                if (pref[1] === method) {
                    xml = xml.replace(`xmlns="http://www.volvocars.com/namespaces/vadis2004/integration/dms/interface"`, ` xmlns:m="http://www.volvocars.com/namespaces/vadis2004/integration/dms/interface"`)
                    var regex = new RegExp('(</?)(' + pref[0] + ')(>| )', 'g');
                    xml = xml.replace(regex, (a, b, c, d) => {
                        console.log(b + 'm:' + c);
                        return b + 'm:' + c + d
                    })
                }
